...
{
    printk(KERN_INFO "fortuneq: Call my_read\n");
    int len;
...
{
    printk(KERN_INFO "fortuneq: Call my_write\n");
    int space_available = (COOKIE_BUF_SIZE - write_index) + 1;

    if (size > space_available)
    {
        printk(KERN_INFO "fortuneq: cookie pot is full\n");
        return -ENOSPC;
    }
...
{
	printk(KERN_INFO "fortuneq: Call my_open\n");
...
{
	printk(KERN_INFO "fortuneq: Сall my_release\n");
	return 0;
}
...
int fortune_init(void)
{
    printk(KERN_INFO "fortuneq: Call my_init\n");

    cookie_buf = (char *) vmalloc(COOKIE_BUF_SIZE);

    if (!cookie_buf)
    {
        printk(KERN_INFO "fortuneq: Not enough memory for the cookie pot\n");
        return -ENOMEM;
    }
    memset(cookie_buf, 0, COOKIE_BUF_SIZE);

    proc_entry = proc_create("fortuneq1", 0666, NULL, &fops);
    if (!proc_entry)
    {
        vfree(cookie_buf);
        printk(KERN_INFO "fortuneq: Couldn't create proc entry\n");
        return -ENOMEM;
    }
    ...
    {
        vfree(cookie_buf);
        printk(KERN_INFO "fortuneq: Couldn't create proc dir\n");
        return -ENOMEM;
    }
    ...
    {
        vfree(cookie_buf);
        printk(KERN_INFO "fortuneq: Couldn't create proc symlink\n");
        return -ENOMEM;
    }

    write_index = 0;

    printk(KERN_INFO "fortuneq: module loaded.\n");
...
void fortune_exit(void)
{
    printk(KERN_INFO "fortuneq: Call my_exit\n");
    if (proc_entry)
        remove_proc_entry("fortuneq1", NULL);
    ...
    printk(KERN_INFO "fortuneq: module unloaded.\n");
}
