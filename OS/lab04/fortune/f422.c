...
{
    printk(KERN_INFO "fortune_buf: Call my_read\n");
    int len;
...
ssize_t fortune_write(struct file *sp_file, const char *buf, size_t size,                              loff_t *offset)
{
    ...
    {
        printk(KERN_INFO "fortune_buf: cookie pot is full\n");
        return -ENOSPC;
    }
...
{
    printk(KERN_INFO "fortune_buf: Call my_open\n");
    return 0;
}

static int my_release(struct inode *inode, struct file *file)
{
    printk(KERN_INFO "fortune_buf: Сall my_release\n");
    return 0;
}
...
int fortune_init(void)
{
    printk(KERN_INFO "fortune_buf: Call my_init\n");
    ...
    {
        vfree(cookie_buf);
        printk(KERN_INFO "fortune_buf: Couldn’t create proc entry\n");
        return -ENOMEM;
    }
    ...
    {
        vfree(cookie_buf);
        printk(KERN_INFO "fortune_buf: Couldn’t create proc dir\n");
        return -ENOMEM;
    }
    ...
    {
        vfree(cookie_buf);
        printk(KERN_INFO "fortune_buf: Couldn’t create proc symlink\n");
        return -ENOMEM;
    }

    write_index = 0;
    printk(KERN_INFO "fortune_buf: module loaded.\n");
    return 0;
}

void fortune_exit(void)
{
    printk(KERN_INFO "fortune_buf: Call my_exit\n");
    if (proc_entry)
    ...
    printk(KERN_INFO "fortune_buf: module unloaded.\n");
}
